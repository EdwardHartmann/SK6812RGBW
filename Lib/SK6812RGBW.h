...
/*--------------------------------------------------------------------
  The SK6812RGBW library is free software: you can redistribute it and/or modify
...
class SK6812RGBW {
 public:
  ...
  SK6812RGBW (uint16_t number_of_leds);// Constuctor 
    ~SK6812RGBW();
 ...
 uint16_t
  ...
  uint16_t
    numLEDs,       // Number of RGBW LEDs in strip
    numBytes;      // Size of 'pixels' buffer

  uint8_t
    brightness,
   *pixels,        // Holds the current LED color values, which the external API calls interact with 12 bytes per pixel + start + end empty bytes
